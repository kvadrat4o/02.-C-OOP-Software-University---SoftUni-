...
public class AbstractHero : IHero, IComparable<AbstractHero>
{
    ...
    {
        Type itemType = typeof(HeroInventory);
        var field = itemType.GetFields(BindingFlags.Instance | BindingFlags.NonPublic).Where(f => f.GetCustomAttribute(typeof(ItemAttribute)).ToString() == nameof(ItemAttribute)).First();
        var itemsDict = (IDictionary<string, IItem>)field.GetValue(this.inventory);
        return itemsDict.Values;
    }
    ...
    public void AddItem(IItem recipe)
    {
        this.inventory.AddCommonItem(recipe);
    }

...
public class Spy
{
    ...
    public string CollectGettersAndSetters(string className)
    {
        StringBuilder sb = new StringBuilder();
        var typeOfHacker = Type.GetType(className);
        var methods = typeOfHacker.GetMethods(BindingFlags.Instance | BindingFlags.Static | BindingFlags.NonPublic | BindingFlags.Public);
        foreach (var getter in methods.Where(g => g.Name.StartsWith("get")))
        {
            sb.AppendLine($"{getter.Name} will return {getter.ReturnType.FullName}");
        }
        foreach (var setter in methods.Where(s => s.Name.StartsWith("set")))
        {

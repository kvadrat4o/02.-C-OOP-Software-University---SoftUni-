namespace BashSoft
{
    ...
    public class StudentsRepository
    {
        ...
        private void ReadData(string fileName)
        {
            ...
            {
                //old regex
                //@"^(?<course>[A-Z][a-zA-Z#+]*_[A-Z][a-z]{2}_\d{4})\s+(?<username>[A-Z][a-z]{0,3}\d{2}_\d{2,4})\s+(?<score>\d+)$"

                string pattern = @"(?<course>[A-Z][a-zA-Z#\+]*_[A-Z][a-z]{2}_\d{4})\s+(?<username>[A-Za-z]+\d{2}_\d{2,4})\s(?<mark>[\s0-9]+)";
